...
const NYCC_NAMED_ENTITIES = {
  ...
  agrave: '\u00e0', egrave: '\u00e8', hellip: '\u2026',
  rsquo: '\u2019', lsquo: '\u2018', ldquo: '\u201c', rdquo: '\u201d',
  ndash: '\u2013', mdash: '\u2014',
};
// NYCC double-encodes its calendar payload, and its titles carry accented names
// ("Angelo Colina: Un Verano en NY (En Espa&ntilde;ol)"). Decode numeric refs and
// the accents we actually see, not just the five XML entities.
function nyccDecode(s) {
  return String(s || '')
    .replace(/&#(\d+);/g, (_, d) => String.fromCharCode(parseInt(d, 10)))
    .replace(/&#x([0-9a-f]+);/gi, (_, h) => String.fromCharCode(parseInt(h, 16)))
    .replace(/&([a-zA-Z]+);/g, (full, name) =>
      Object.prototype.hasOwnProperty.call(NYCC_NAMED_ENTITIES, name) ? NYCC_NAMED_ENTITIES[name] : full);
}

// NYCC's calendar has no lineup field — the bill, when there is one, is inside the
// event title. Three shapes actually occur:
//   "Elon Gold, Mike Britt, Maddy Smith"          -> a bare comma-separated bill
//   "Good Eggs ft: Ryan Hamilton, Gary Vider"     -> show name, then the bill
//   "Yerrrlywood" / "Closed For Private Event"    -> a show name, NOT a person
// The old rule inverted the middle case (it threw those lineups away) and treated
// every title without a colon as a one-name lineup, which is where chips like
// "Closed For Private Event" and "New Jokes & Newbies" came from. Only emit names
// we're confident about: an explicit ft:/featuring/presents tail, or a bare list of
// two or more comma-separated names. A lone title is a show name and yields [].
// Show copy that shows up inside a title but is never a bookable performer.
const NYCC_NON_NAME_WORDS = new Set([
  'the', 'a', 'an', 'and', 'with', 'ft', 'feat', 'featuring', 'presents', 'present',
  'show', 'showcase', 'shows', 'night', 'nights', 'comedy', 'club', 'mic', 'open',
  'live', 'series', 'final', 'finals', 'festival', 'fest', 'lineup', 'line-up',
  'party', 'special', 'edition', 'hour', 'late', 'early', 'matinee', 'brunch',
  'jam', 'roast', 'hosted', 'host', 'guest', 'guests', 'surprise', 'more', 'tbd',
  'tba', 'stand-up', 'standup', 'improv', 'closed', 'private', 'event', 'tour',
  'benefit', 'anniversary', 'showdown', 'battle', 'contest', 'competition',
]);
const NYCC_NAME_PARTICLE = /^(?:de|del|la|le|van|von|da|di|du|el|bin|al|st|mc|o)$/i;

// A bookable name: two to four words, no digits, each word capitalised (or a
// lowercase particle), and none of them show copy. Deliberately strict — a wrong
// chip becomes a favouritable, searchable, Top-Pick-scoring phantom comedian.
function nyccLooksLikeName(s) {
  const t = String(s || '').trim();
  if (!t || t.length > 40 || /\d/.test(t)) return false;
  const words = t.split(/\s+/);
  if (words.length < 2 || words.length > 4) return false;
  return words.every(w => {
    const bare = w.replace(/[^A-Za-z'À-ɏ.-]/g, '').replace(/[.']+$/, '');
    if (!bare) return false;
    if (NYCC_NON_NAME_WORDS.has(bare.toLowerCase())) return false;
    return NYCC_NAME_PARTICLE.test(bare) || /^[A-ZÀ-Þ]/.test(bare);
  });
}

function nyccExtractLineup(title) {
  const t = String(title || '').trim();
  if (!t) return [];
  // An explicit ft:/featuring/presents tail is the club stating the bill, so a
  // single announced name there is enough. A bare comma list or a "Show: names"
  // tail is inferred, so it needs two real names before we believe it.
  const ft = t.match(/\s(?:ft|feat|featuring|presents|presented\s+by|starring)\.?\s*:?\s+(.+)$/i);
  let listPart = null;
  let announced = false;
  if (ft) { listPart = ft[1]; announced = true; }
  else if (t.includes(',')) { listPart = t; }
  else {
    const colon = t.match(/^[^:]{2,}:\s+(.+)$/);
    if (colon) listPart = colon[1];
  }
  if (!listPart) return [];
  const parts = listPart
    ...
    .filter(Boolean);
  const names = [...new Set(parts.filter(nyccLooksLikeName))];
  if (!names.length) return [];
  if (!announced && names.length < 2) return [];
  return names;
}
...
module.exports = { scrapeGotham, scrapeStandupNY, scrapeTheStand, scrapeNYCC, scrapeUnionHall, applyStandupnyEnrichment, nyccExtractLineup, nyccLooksLikeName, nyccDecode };
