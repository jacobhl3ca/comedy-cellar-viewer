...
async function scrapeStandupNY() {
  const today = todayNY();
  const query = `{ paginatedEvents(arguments:{accountIds:[2535], startDate:"${today}", limit:200}){ collection { id name date startTime status images } } }`;
  ...
  return collection
    ...
    .map(e => ({
      title: gothamDecode(e.name), date: e.date, time: standupnyFmtTime(e.startTime),
...
module.exports = { scrapeGotham, scrapeStandupNY, scrapeTheStand, scrapeNYCC };
